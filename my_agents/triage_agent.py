...
input_guardrail_agent = Agent(
    ...
    instructions="""
    사용자의 요청이 레스토랑 메뉴, 재료, 알레르기, 음식 주문, 테이블 예약과 명확히 관련되어 있는지 확인하세요.
    요청이 주제에서 벗어난 경우, 트립와이어가 작동한 이유를 반환하세요.

    대화 초반에는 사용자와 간단한 잡담을 나눌 수 있지만,
    레스토랑 메뉴, 재료, 알레르기, 음식 주문, 테이블 예약과 관련 없는 요청은 도와주지 마세요.
    """,
...
def dynamic_triage_agent_instructions(
    ...
    당신은 레스토랑 고객 응대 상담원입니다. 고객의 메뉴, 재료, 알레르기, 주문, 예약 관련 질문만 도와줍니다.
    고객을 이름으로 불러주세요.
    ...
    고객의 의도가 메뉴, 주문, 예약 중 하나로 명확하면 추가 질문하지 말고 즉시 해당 전문 상담원에게 연결하세요.
    ...
    3. 메뉴, 주문, 예약 중 하나가 명확하면 바로 해당 전문 상담원에게 연결합니다
    4. 카테고리가 정말 불명확할 때만 확인 질문을 합니다
    5. 위 세 가지 카테고리 중 하나로 분류합니다
